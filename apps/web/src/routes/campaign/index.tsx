...
export const Route = createFileRoute("/campaign/")({
	...
	loader: async () => {
		try {
			...
			const message = getApiErrorMessage(error, "Failed to load campaign");
				...
				<div className="mb-24 grid grid-cols-1 gap-8 md:grid-cols-2">
					{Object.entries(data).map(([campaignId, campaign]) => {
						...
						const requiredCampaign = user?.campaign_progress?.find(
							(cp) => cp.campaign_id === typedCampaign?.requirement,
						);

						const isUnlocked =
							!typedCampaign.requirement || requiredCampaign?.isCompleted;
						...
						return (
							<div
								key={campaignId}
								className="group theme-accent-border hover:theme-accent-border relative flex flex-col border-2 bg-black shadow-lg transition-colors"
							>
								<div className="theme-accent-border/50 theme-accent-bg/10 border-b p-6">
									<div className="mb-3 flex items-start justify-between">
										<span className="font-mono text-gray-400 text-xs uppercase tracking-widest">
											{campaignId.replace("_", " ").toUpperCase()}
										</span>
								...
								<div className="flex-1 space-y-2 bg-zinc-900/20 p-4">
									{Object.entries(typedCampaign?.levels).map(
										([levelId, level]: [string, ICampaignLevel]) => {
											const isLevelCompleted =
												campaignProgress?.levelsCompleted.includes(levelId);
												...
												<Link
													...
													className={`group/item flex cursor-pointer items-center justify-between border p-3 transition-all ${
														isLevelCompleted
															...
															: "theme-accent/40 hover:theme-accent-border hover:theme-accent-bg/10 border-zinc-800 bg-black"
													}
                                                `}
												>
													<div className="flex items-center gap-4">
														...
														>
															{levelId.toUpperCase().replace("LEVEL_", "0")}
														</span>
													...
													{isLevelCompleted ? (
														...
														<Circle
															size={16}
															className="group-hover/item:theme-accent text-zinc-700 transition-colors"
														/>
													)}
												</Link>
											);
